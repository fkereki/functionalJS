...
const listSize2 = list => {
    const accumSize = (accum, list) =>
        list((head, tail) => accumSize(accum + 1, tail), () => accum);
...
const listSearch = (list, value) =>
    list(
        (head, tail) => head === value || listSearch(tail, value),
        () => false
    );

const listGetAt = (list, pos) =>
    list(
        (head, tail) => (pos === 0 ? head : listGetAt(tail, pos - 1)),
        () => throwEmptyListError()
    );
...
const listMap = (list, fn) =>
    list((head, tail) => NewList(fn(head), listMap(tail, fn)), EmptyList);
...
const listReduce = (list, fn, accum) =>
    list(
...
const queueIsEmpty = queue =>
    queue((front, back) => isEmptyList(front), () => true);

const queueHead = queue =>
    queue((front, back) => listHead(front), () => undefined);
...
const queueRemoveFromFront = queue =>
    queue(

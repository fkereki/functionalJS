const NewTreap = (value, priority, left, right) => (f, g) => f(value, priority, left, right);

const EmptyTreap = () => (f, g) => g();

const treapIsEmpty = treap => treap((value, priority, left, right) => false, () => true);

const treapValue = treap => treap((value, priority, left, right) => value, () => undefined);

const treapPriority = treap => treap((value, priority, left, right) => priority, () => undefined);

const treapLeft = treap => treap((value, priority, left, right) => left, () => undefined);

const treapRight = treap => treap((value, priority, left, right) => right, () => undefined);
...
const treapInsert = (newValue, newPriority, treap) =>
    treap(
        (value, priority, left, right) => {
            ...
            if (newValue <= value) {
                candidate = NewTreap(value, priority, treapInsert(newValue, newPriority, left), right);
                ...
                if (treapPriority(leftTree) > priority) {
                    candidate = NewTreap(
                        ...
                        treapLeft(leftTree),
                        NewTreap(value, priority, treapRight(leftTree), right)
                    );
                }
            } else {
                candidate = NewTreap(value, priority, left, treapInsert(newValue, newPriority, right));
                ...
                if (treapPriority(rightTree) > priority) {
                    candidate = NewTreap(
                        ...
                        NewTreap(value, priority, left, treapLeft(rightTree)),
...
const treapSearch2 = (findValue, treap) =>
    treap(
        (value, priority, left, right) =>
            findValue === value || treapSearch2(findValue, findValue < value ? left : right),
        () => false
    );
...
const treapSearchForNode = (findValue, treap) =>
    treap(
        (value, priority, left, right) =>
            findValue === value ? tree : treapSearchForNode(findValue, findValue < value ? left : right),
        () => null
    );
    ...
    treap(
        (value, priority, left, right) => {
            if (valueToDelete < value) {
                return NewTreap(value, priority, treapDelete(left, valueToDelete), right);
            } else if (valueToDelete > value) {
                return NewTreap(value, priority, left, treapDelete(right, valueToDelete));
                ...
                return NewTreap(
                    treapValue(right),
                    treapPriority(right),
                    treapDelete(NewTreap(value, priority, left, treapLeft(right)), valueToDelete),
                    treapRight(right)
                );
            } else {
                return NewTreap(
                    ...
                    treapPriority(left),
                    treapLeft(left),
                    treapDelete(NewTreap(value, priority, treapRight(left), right), valueToDelete)
                );
            }
        },
        () => treap
